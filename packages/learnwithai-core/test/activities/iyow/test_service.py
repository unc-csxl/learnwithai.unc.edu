...
def _make_service(
    activity_svc: MagicMock | None = None,
    iyow_activity_repo: MagicMock | None = None,
) -> IyowActivityService:
    return IyowActivityService(
        activity_svc=activity_svc or MagicMock(spec=ActivityService),
...
def test_create_iyow_activity_succeeds_for_instructor() -> None:
    activity_svc = MagicMock(spec=ActivityService)
    created_activity = _make_activity()
    activity_svc.create_activity.return_value = created_activity
    ...
    svc = _make_service(activity_svc, iyow_repo)
    ...
    activity_svc.create_activity.assert_called_once()
    iyow_repo.create.assert_called_once()


def test_create_iyow_activity_succeeds_for_ta() -> None:
    activity_svc = MagicMock(spec=ActivityService)
    activity_svc.create_activity.return_value = _make_activity()
    iyow_repo = MagicMock(spec=IyowActivityRepository)
...
def test_create_iyow_activity_raises_for_student() -> None:
    activity_svc = MagicMock(spec=ActivityService)
    activity_svc.create_activity.side_effect = AuthorizationError("Insufficient permissions")

    svc = _make_service(activity_svc=activity_svc)
...
def test_create_iyow_activity_raises_for_non_member() -> None:
    activity_svc = MagicMock(spec=ActivityService)
    activity_svc.create_activity.side_effect = AuthorizationError("Not a member of this course")
...
def test_update_iyow_activity_succeeds() -> None:
    ...
    activity_svc.update_activity.return_value = activity
    iyow_repo = MagicMock(spec=IyowActivityRepository)
    detail = _make_iyow_detail()
    iyow_repo.get_by_activity_id.return_value = detail
    ...
    activity_svc.update_activity.assert_called_once()
    iyow_repo.update.assert_called_once()


def test_update_iyow_activity_raises_for_student() -> None:
    activity_svc = MagicMock(spec=ActivityService)
    activity_svc.update_activity.side_effect = AuthorizationError("Insufficient permissions")
