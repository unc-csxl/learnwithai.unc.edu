...
def test_configure_broker_builds_and_registers_rabbitmq_broker() -> None:
    ...
    # Act
    with (
        patch("learnwithai_jobqueue.broker.get_settings", return_value=settings),
        patch("learnwithai_jobqueue.broker.RabbitmqBroker", return_value=expected_broker) as rabbitmq_broker_mock,
        patch("learnwithai_jobqueue.broker.dramatiq.set_broker") as set_broker_mock,
    ):
